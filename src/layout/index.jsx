...
const MyLayout = ({
  ...
  const [enquiryImgCost, setEnquiryImgCost] = useState() // plotreel云-每张图扣除的算币
  const [enquiryVideoCost, setEnquiryVideoCost] = useState() // plotreel云-每个视频扣除的算币
  ...
  instance.interceptors.response.use(
    (response) => {
      ...
      if (response?.data?.resCode === NETWORK_ERR) {
        message.error('plotreel连接失败，请检查网络')
      }
      return response
    },
    async (e) => {
        ...
        <DragHeader globalToken={globalToken}>
            ...
            {isInit && (
              ...
              >
                {!drawingSdState ? (
                  ...
                  <div style={{ color: 'green' }}>plotreel云-{modelName}</div>
                ) : drawingModel === 'custom' ? (
          ...
          <div>
            <Space size="small">
              ...
              >
                <Avatar style={{ verticalAlign: 'middle' }} alt="plotreel" src={defaultAvator}>
                  名
                </Avatar>
              </div>
            </Space>
