...
const Settings = ({
  ...
  // 云端
  const [cloudSdState, setCloudSdState] = useState(false) // plotreel云是否启动
  ...
  const changeVideoModelForCloud = async (value) => {
    ...
    if (res?.data?.resCode == SUCCESS_CODE) {
      ...
      message.success(`plotreel云${target.label}连接成功`)
    } else {
      setCloudSdState(false)
      setDrawingSdState(false)
      message.error(`plotreel云${target?.label}连接失败，请稍后重试`)
    }
